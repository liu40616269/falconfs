...
static int WaitForResponse(SyncContext &ctx)
{
    std::unique_lock<std::mutex> lock(ctx.mtx);
    if (!ctx.cv.wait_for(lock, std::chrono::milliseconds(TEST_TIMEOUT_MS), [&ctx] { return ctx.done; })) {
        printf("[TEST] ERROR: Request timeout!\n");
        return -1;
    }
    return 0;
}
...
/*
 * 测试: 并发创建同名文件
 * [Worker-only] 测试两个线程同时创建同一个文件
 * 用于复现 InterruptHoldoffCount 断言失败问题
 * 预期：一个成功，一个返回 FILE_EXISTS
 */
static bool TestConcurrentSameFileCreation()
{
    ...
    printf("  Running %d iterations with %d threads creating same file...\n", ITERATIONS, THREAD_COUNT);

    for (int iter = 0; iter < ITERATIONS; iter++) {
        std::vector<std::thread> threads;
        std::string file_path = "/same_file_test_" + std::to_string(iter) + ".txt";
        std::atomic<int> iter_success(0);
        std::atomic<int> iter_exists(0);
        std::atomic<int> iter_error(0);

        auto create_same_file_worker = [&]() {
            CreateResponse create_resp;
            int status = DoCreate(file_path, &create_resp);
            ...
            if (status == 0) {
                ...
                iter_error++;
                printf("  [Iter %d] Unexpected error: status=%d\n", iter, status);
            }
        };

        for (int i = 0; i < THREAD_COUNT; i++) {
            threads.emplace_back(create_same_file_worker);
        }
        ...
        success_count += iter_success.load();
        exists_count += iter_exists.load();
        error_count += iter_error.load();

        // 每次迭代应该恰好有1个成功
        if (iter_success.load() != 1) {
            printf("  [Iter %d] Warning: expected 1 success, got %d (exists=%d, error=%d)\n",
                   iter, iter_success.load(), iter_exists.load(), iter_error.load());
        }
    ...
    printf("  Results over %d iterations (%d threads each):\n", ITERATIONS, THREAD_COUNT);
    printf("    Success (first create): %d (expected: %d)\n", success_count.load(), ITERATIONS);
    printf("    FILE_EXISTS (expected): %d (expected: %d)\n", exists_count.load(), ITERATIONS * (THREAD_COUNT - 1));
    printf("    Errors (unexpected):    %d\n", error_count.load());

    TEST_ASSERT_MSG(error_count.load() == 0,
                    "Unexpected errors occurred: %d", error_count.load());
...
static void RunAllTests()
{
    ...
    if (g_is_cn_node) {
        ...
        TestConcurrentSameFileCreation();  // 并发创建同名文件测试（复现 InterruptHoldoffCount 问题）
...
int plugin_work(FalconPluginData *data)
{
    ...
    printf("[FalconMetaServiceTestPlugin] Initializing FalconMetaService on port %d...\n", g_node_port);
    fflush(stdout);

    g_meta_service = FalconMetaService::Instance();
    if (!g_meta_service->IsInitialized()) {
        if (!g_meta_service->Init(g_node_port, 1)) {
            printf("[FalconMetaServiceTestPlugin] ERROR: Failed to init FalconMetaService\n");
            printf("[FalconMetaServiceTestPlugin] This might not be a CN node, skipping tests.\n");
            fflush(stdout);
            return 0;
        }
    }

    printf("[FalconMetaServiceTestPlugin] FalconMetaService initialized successfully\n");
    fflush(stdout);
