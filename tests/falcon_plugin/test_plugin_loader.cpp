...
TEST_F(PluginLoaderUT, VerifyInlinePluginHandling)
{
    ...
    falcon_plugin_init_func_t init_func =
        (falcon_plugin_init_func_t)dlsym(dl_handle, FALCON_PLUGIN_INIT_FUNC_NAME);
...
TEST_F(PluginLoaderUT, VerifyPluginUnloadingBehavior)
{
    ...
    falcon_plugin_cleanup_func_t cleanup_func =
        (falcon_plugin_cleanup_func_t)dlsym(dl_handle, FALCON_PLUGIN_CLEANUP_FUNC_NAME);

    ASSERT_NE(init_func, nullptr);
    ASSERT_NE(cleanup_func, nullptr);
...
TEST_F(PluginLoaderUT, MultiplePluginLoadUnload)
{
    ...
    for (const auto& path : plugin_paths) {
        ...
        if (handle != nullptr) {
            ...
            falcon_plugin_init_func_t init_func =
                (falcon_plugin_init_func_t)dlsym(handle, FALCON_PLUGIN_INIT_FUNC_NAME);
            EXPECT_NE(init_func, nullptr);
        }
    }
