...
FalconMetaServiceJob::~FalconMetaServiceJob() { CleanupResponseData(m_response); }
...
bool FalconMetaServiceJob::IsAllowBatchProcess() { return IsAllowBatchOperation(m_request.operation); }

bool FalconMetaServiceJob::IsEmptyRequest() { return false; }

int FalconMetaServiceJob::GetReqServiceCnt() { return 1; }

size_t FalconMetaServiceJob::GetReqDatasize() { return m_request_buffer.size(); }
...
{
    if (index != 0) {
        fprintf(stderr, "[WARNING] [FalconMetaService] invalid service index: %d\n", index);
